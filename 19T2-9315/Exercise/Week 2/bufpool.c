...
// since we don't actually need to maitain a usedList
...
struct bufPool {
    ...
    int nrequests;  // LRU, MRU, Cycle
    int nreleases;
    int nhits;
    int nreads;
    int nwrites;
    int *freeList;  // implements replacement strategy
    int nfree;
...
// showPoolState(pool)
// - display printable representaion of buffer pool on stdout
