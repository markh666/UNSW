...
def is_word_word_ladder(word_1, word_2, candidate_ladder):
    ...
    if word_1 in lexicon and word_2 in lexicon and candidate_ladder[0] == word_1 and candidate_ladder[-1] == word_2:
        ...
        for i in candidate_ladder[1:]:
            if not i in closest_words[current] :
                return False
            if i in closest_words[current] and i != word_2 and word_2 in closest_words[current]:
                return False
            current = i
    else:
        return False
...
##                if not i in closest_words[current]:
##                    return False
##                # when above if is not false(passed, satisfyed )
##                if (i in candidate_ladder[current] and  i != word_2 and word_2 in closest_words[current]: # count = 0 if word2 in closet before: count +=1. if count ==1 is true. if count != 1 then it is false
##                    return False
##                current = i
##        else:
##            return False
