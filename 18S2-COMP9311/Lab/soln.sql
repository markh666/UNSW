...
create function check_manager()
	returns trigger as $uniq_dep$
    declare
	tfn text;
    begin
	for tfn in (select manager from departments) loop
		if ((select count(*) from WorksFor w where w.employee=tfn) > 1) then
                	raise exception 'a manager cannot work in more than one department';
		end if;
	end loop;
	return NEW;
    end;
$uniq_dep$ language plpgsql;
 
create trigger check_manager after insert or update on WorksFor
    for each row execute procedure check_manager();
