    ...
    def keep_every_second_element(self):
        ...
        if not self.head:
            ...
            while node.next_node.next_node:
                ...
                node = second
                if not node.next_node:
                    self.tail = node # 1 2 3 后面什么都没有的时候
                    return # terminal this function
            node.next_node = None #断开了和下一个node的联系,不然会打印出来 1 2 3 4 但是没有5
            self.tail = node  #把tail变成node  1 2 3 后面只有 4 的时候
        return
